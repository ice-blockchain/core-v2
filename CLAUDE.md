...
|---|---|---|
| `frontend/` | React Native mobile app, all internal packages | ion-app |
| `backend/` | All backend services | ion-backend |

Both share typed API contracts via `@ion/api-contracts`.
...
### Folder Structure — Frontend Package
```
...
- Backend: use `AppError` from `backend/shared/utils/errors.ts` with status code + error code.
