...
// ExtractLeafHash navigates a Merkle proof cell to extract the hash stored
// at the given leaf index. The proof must have been generated by GenerateMerkleProof.
// Returns the 32-byte SHA256 hash stored in the leaf cell.
func ExtractLeafHash(proofCell *cell.Cell, leafIndex int, totalLeaves int) ([32]byte, error) {
	if totalLeaves <= 0 {
		return [32]byte{}, fmt.Errorf("totalLeaves must be positive")
	}
	if leafIndex < 0 || leafIndex >= totalLeaves {
		return [32]byte{}, fmt.Errorf("leafIndex %d out of range [0, %d)", leafIndex, totalLeaves)
	}

	// MerkleProof exotic cell has one ref: the inner tree root.
	refs := proofCell.RefsNum()
	if refs == 0 {
		return [32]byte{}, fmt.Errorf("proof cell has no refs")
	}
	current, err := proofCell.PeekRef(0)
	if err != nil {
		return [32]byte{}, fmt.Errorf("peek proof inner cell: %w", err)
	}

	// Navigate the binary tree to the leaf.
	treeSize := nextPowerOfTwo(totalLeaves)
	for treeSize > 1 {
		mid := treeSize / 2
		if current.RefsNum() < 2 {
			return [32]byte{}, fmt.Errorf("expected 2 refs at internal node, got %d", current.RefsNum())
		}
		if leafIndex < mid {
			current, err = current.PeekRef(0)
			treeSize = mid
		} else {
			current, err = current.PeekRef(1)
			leafIndex -= mid
			treeSize -= mid
		}
		if err != nil {
			return [32]byte{}, fmt.Errorf("navigate proof tree: %w", err)
		}
	}

	// Leaf cell contains 256 bits = 32 bytes of SHA256 hash.
	if current.BitsSize() < 256 {
		return [32]byte{}, fmt.Errorf("leaf cell has %d bits, expected 256", current.BitsSize())
	}
	slice := current.BeginParse()
	hashBytes, err := slice.LoadSlice(256)
	if err != nil {
		return [32]byte{}, fmt.Errorf("read leaf hash: %w", err)
	}
	var h [32]byte
	copy(h[:], hashBytes)
	return h, nil
