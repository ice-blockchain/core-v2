...
export function BottomNavBarSheet({ isVisible, onClose, title, actions, testID }: BottomNavBarSheetProps) {
  const scale = useTheme().scale.scaleSize;

  return (
    <BottomSheet isVisible={isVisible} onClose={onClose} title={title} {...(testID ? { testID } : {})}>
