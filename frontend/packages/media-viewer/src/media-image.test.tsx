...
describe('MediaImage', () => {
  it('renders with the provided URI', () => {
    const { getByTestId } = render(<MediaImage source={baseSource} />);
  ...
  it('displays blurhash as placeholder', () => {
    ...
    const { getByTestId } = render(<MediaImage source={source} />);
    const image = getByTestId('expo-image');
  ...
  it('fires onLoad callback', () => {
    ...
    const { getByTestId } = render(<MediaImage source={baseSource} onLoad={onLoad} />);
    const image = getByTestId('expo-image');
    image.props.onLoad();
  ...
  it('fires onError callback with Error object', () => {
    ...
    const { getByTestId } = render(<MediaImage source={baseSource} onError={onError} />);
    const image = getByTestId('expo-image');
    image.props.onError();
