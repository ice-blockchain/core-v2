...
describe('mapIdentityError', () => {
  it('maps USER_NOT_FOUND to account not found message', () => {
    ...
    expect(result.userMessage).toBe('auth:errorUserNotFound');
  });
  ...
  it('maps USER_ALREADY_EXISTS to identity key taken message', () => {
    ...
    expect(result.userMessage).toBe('auth:errorUserAlreadyExists');
  });
  ...
  it('maps INVALID_CREDENTIALS to incorrect password message', () => {
    ...
    expect(result.userMessage).toBe('auth:errorInvalidCredentials');
  });
  ...
  it('maps PASSKEY_CANCELLED to cancelled message', () => {
    ...
    expect(result.userMessage).toBe('auth:errorPasskeyCancelled');
  });
  ...
  it('maps PASSKEY_NOT_AVAILABLE to not available message', () => {
    ...
    expect(result.userMessage).toBe('auth:errorPasskeyNotAvailable');
  });
  ...
  it('maps PASSKEY_VALIDATION_FAILED to validation failed message', () => {
    ...
    expect(result.userMessage).toBe('auth:errorPasskeyValidationFailed');
  });
  ...
  it('maps NETWORK_ERROR to connection failed message', () => {
    ...
    expect(result.userMessage).toBe('auth:errorNetworkError');
  });
  ...
  it('maps USER_DEACTIVATED to deactivated message', () => {
    ...
    expect(result.userMessage).toBe('auth:errorUserDeactivated');
  });
  ...
  it('maps TOKEN_EXPIRED to session expired message', () => {
    ...
    expect(result.userMessage).toBe('auth:errorTokenExpired');
  });
  ...
  it('maps UNAUTHENTICATED to auth required message', () => {
    ...
    expect(result.userMessage).toBe('auth:errorUnauthenticated');
  });
  ...
  it('maps UNKNOWN identity error to generic message', () => {
    ...
    expect(result.userMessage).toBe('auth:errorUnknown');
  });
